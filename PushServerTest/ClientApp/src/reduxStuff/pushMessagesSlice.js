...
export const sendPushMessage = createAsyncThunk('pushMessages/sendPushMessage', async (pushMessage) => {
  console.log("sendPushMessage: " + JSON.stringify(pushMessage));
...
export const removeUserData = createAsyncThunk('pushMessages/removeUserData', async (userData, thunkAPI) => {
  const ret = await PushApiClient.deleteUserData(userData);
  thunkAPI.dispatch(fetchAll());
  return ret;
});
...
export const restoreApiClient = createAsyncThunk('pushMessages/restoreApiClient', async (apiClientData, thunkAPI) => {
  const ret =  await PushApiClient.restoreApiClient(apiClientData);
  thunkAPI.dispatch(fetchAll());
  return ret;
});
...
export const editApiDescription = createAsyncThunk('pushMessages/editApiDescription', async (apiClientData, thunkAPI) => {
  const ret = await PushApiClient.editApiDescription(apiClientData);
  thunkAPI.dispatch(fetchAll());
  return ret;
});
